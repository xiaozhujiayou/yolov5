...
def yolov8_post_process(outputs, strides, conf_thresh, nms_thresh):
    ...
    for out in outputs:
        ...
        if out.shape[1] == (REG_MAX * 4): # Box regression
            outputs_by_scale[stride]['reg'] = out
        elif out.shape[1] == NUM_CLASSES: # Classification
            outputs_by_scale[stride]['cls'] = out
        else: # Objectness (if exists)
             outputs_by_scale[stride]['obj'] = out


    # 2. 遍历每个尺度进行解码
    for stride in strides:
        if stride not in outputs_by_scale or 'reg' not in outputs_by_scale[stride] or 'cls' not in outputs_by_scale[stride]:
            continue
        
        reg_tensor = outputs_by_scale[stride]['reg'] # (1, 64, H, W)
        cls_tensor = outputs_by_scale[stride]['cls'] # (1, NUM_CLASSES, H, W)
        # 您的模型有独立的 objectness 分支
        obj_tensor = outputs_by_scale[stride].get('obj') # (1, 1, H, W)
        ...
        obj_preds = obj_tensor.squeeze(0).transpose(1, 2, 0).flatten() if obj_tensor is not None else None

        # DFL 解码
        dist_reg = np.arange(REG_MAX, dtype=np.float32)
        e_x = np.exp(reg_preds - np.max(reg_preds, axis=-1, keepdims=True))
        reg_probs = e_x / e_x.sum(axis=-1, keepdims=True)
        ltrb_offsets = np.sum(reg_probs * dist_reg, axis=-1)

        # 类别分数
        cls_scores = _sigmoid(cls_preds)
        obj_scores = _sigmoid(obj_preds) if obj_preds is not None else 1.0 # 如果没有obj分支，则默认为1
...
class YoloDetectorNode:
    ...
    def run_inference_loop(self):
        ...
        frame_count = 0 
        strides = [8, 16, 32] # YOLOv8 standard strides
